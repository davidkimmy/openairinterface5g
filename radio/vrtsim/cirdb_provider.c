                                ...
                                channel_desc_t **channel_desc_out)
{
  ...
  G.snapshot_tmp = calloc_or_fail(G.snapshot_tmp_bytes);
  ...
  for (int i = 0; i < NUM_TAPS_BUFFERS; i++) {
    ...
    G.bufs[i].ch = calloc_or_fail(sizeof(channel_desc_t));
    G.bufs[i].ch->ch_ps = calloc_or_fail((size_t)G.n_rx * G.n_tx * sizeof(struct complexf *));
  ...
  cirdb_g *p = calloc_or_fail(sizeof(cirdb_g));
  *p = G;
  return p;
}

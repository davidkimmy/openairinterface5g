...
// gtest.h MUST precede OAI headers: common_lib.h -> log.h -> T.h defines a function-like
// macro T(...) even when T-Tracer is disabled, which clobbers GoogleTest's `template <typename T>`.
// (Branch-specific: this branch's T.h leaks the macro; develop builds tests at C++17 globally.)

...
#include "SIMULATION/TOOLS/noise_device.h"
...
// irrelevant (like rfsimulator): the peer may come up much later than the client - e.g. a mode-1
// relay opens its PC5 IPC only after its Uu link reaches RRC CONNECTED (~10-25s). Bounded (not
// while(1)) so a genuinely-absent server still fails with a clear message in a tolerable time.
...
// Sidelink (PC5) defaults: a channel/descriptor distinct from Uu so both can run in parallel.
#define DEFAULT_CHANNEL_NAME_SL "vrtsim_channel_sl"
#define DEFAULT_DESCRIPTOR_SL "/tmp/vrtsim_connection_sl"
#define TPOOL_HLP "Thread pool for channel modelling. Only used if CUDA support is disabled."

// Backported from upstream channelmod (absent on this branch): sentinel meaning "noise disabled".
// On this branch the noise power (in dBFS) is provided as a vrtsim param and read directly from
// vrtsim_state->noise_power_dBFS, instead of the upstream channelmod-section getter.
...
#define VRTSIM_PARAMS_DESC \
  { \
     ...
     {"noise_power_dBFS",       "Noise power in dBFS (100 = disabled)\n", 0, .iptr = &vrtsim_state->noise_power_dBFS, .defintval = INVALID_DBFS_VALUE, TYPE_INT, 0}, \
     /* Sidelink (PC5) link: a shared-memory channel distinct from Uu, started via trx_start_func_sl */ \
     {"role_sl",                "Sidelink (PC5) role: server or client\n", 0, .strptr = &role_sl, .defstrval = ROLE_CLIENT_STRING, TYPE_STRING, 0}, \
     {"shm_channel_name_sl",    "Sidelink (PC5) shared memory channel name\n", 0, .strptr = &vrtsim_state->shm_channel_name_sl, .defstrval = DEFAULT_CHANNEL_NAME_SL, TYPE_STRING, 0}, \
     {"connection_descriptor_sl", "Sidelink (PC5) connection descriptor path\n", 0, .strptr = &vrtsim_state->connection_descriptor_sl, .defstrval = DEFAULT_DESCRIPTOR_SL, TYPE_STRING, 0}, \
...
typedef struct {
  ...
  // Sidelink (PC5) link identity, used by vrtsim_connect_sl (distinct channel from Uu).
  int role_sl;
  char *connection_descriptor_sl;
  char *shm_channel_name_sl;
  ...
  int chanmod;
  // Last noise amplitude pushed to the (process-global) noise device; lets vrtsim_write_with_chanmod
  // detect a runtime change of channel_desc->noise_power_dB (e.g. via telnet) and regenerate noise only then.
  int16_t cur_noise_power;
  // Per-device sample history for the chanmod convolution. MUST be per-device (not a function-local
  // static): a mode-1 relay runs TWO vrtsim instances (Uu + PC5) whose TX threads call
  // vrtsim_write_with_chanmod concurrently; a shared static races and intermittently crashes.
  c16_t saved_samples[MAX_NUM_ANTENNAS_TX][SAVED_SAMPLES_LEN] __attribute__((aligned(32)));
  double rx_freq;
  double tx_bw;
  int tx_num_channels;
  int rx_num_channels;
...
{
  // Branch load_channellist has no center-frequency parameter (dropped vs upstream).
  load_channellist(vrtsim_state->tx_num_channels, nb_rx, vrtsim_state->sample_rate, vrtsim_state->tx_bw);
  ...
  // Seed the live noise level: if --vrtsim.noise_power_dBFS was given it overrides the conf's noise_power_dB;
  // otherwise the conf value stays. From here on the runtime noise is read from channel_desc->noise_power_dB
  // (so the telnet "channelmod modify <id> noise_power_dB <v>" command adjusts vrtsim noise live).
  if (vrtsim_state->noise_power_dBFS != INVALID_DBFS_VALUE)
    channel_desc->noise_power_dB = vrtsim_state->noise_power_dBFS;
  LOG_A(HW,
...
static void vrtsim_readconfig(vrtsim_state_t *vrtsim_state)
{
  ...
  int ret = config_get(vrtsim_params, sizeofArray(vrtsim_params), VRTSIM_SECTION);
  ...
  // Sidelink (PC5) role; only consulted by vrtsim_connect_sl for the rfdevice_sl instance.
  if (strncmp(role_sl, ROLE_CLIENT_STRING, strlen(ROLE_CLIENT_STRING)) == 0) {
    vrtsim_state->role_sl = ROLE_CLIENT;
  } else if (strncmp(role_sl, ROLE_SERVER_STRING, strlen(ROLE_SERVER_STRING)) == 0) {
    vrtsim_state->role_sl = ROLE_SERVER;
  } else {
    AssertFatal(false, "Invalid role_sl configuration\n");
  }
...
static void parse_ue_config(vrtsim_state_t *vrtsim_state)
{
  ...
  for (int i = 0; i < vrtsim_state->num_ues; i++) {
    ...
    config_get(ue_params, sizeofArray(ue_params), prefix);
...
static int vrtsim_connect(openair0_device_t *device)
{
  ...
  if (vrtsim_state->role == ROLE_SERVER) {
    ...
    // Same launch-order tolerance for attaching the shm segment (server may create it late).
    vrtsim_state->channel = shm_td_iq_channel_connect(vrtsim_state->shm_channel_name, VRTSIM_PEER_WAIT_SEC);
    vrtsim_state->last_received_sample = shm_td_iq_channel_get_current_sample(vrtsim_state->channel);
  }
...
// Sidelink (PC5) start hook. rfdevice_sl is its own device instance with its own private
// vrtsim_state (separate device_init / openair0_device_load in executables/nr-ue.c). Repoint this
// instance's channel identity to the PC5 (_sl) config, then reuse vrtsim_connect. This keeps Uu and
// PC5 on distinct shared-memory channels (analogous to rfsimulator's separate serverportsl) with no
// duplication of the connect logic. Safe because it only mutates the SL device's own state.
static int vrtsim_connect_sl(openair0_device_t *device)
{
  vrtsim_state_t *vrtsim_state = (vrtsim_state_t *)device->priv;
  vrtsim_state->role = vrtsim_state->role_sl;
  vrtsim_state->shm_channel_name = vrtsim_state->shm_channel_name_sl;
  vrtsim_state->connection_descriptor = vrtsim_state->connection_descriptor_sl;
  LOG_A(HW, "VRTSIM: starting sidelink (PC5) link on channel %s as %s\n",
        vrtsim_state->shm_channel_name,
        vrtsim_state->role == ROLE_SERVER ? "server" : "client");
  return vrtsim_connect(device);
}
...
static int vrtsim_write_with_chanmod(vrtsim_state_t *vrtsim_state,
                                     ...
                                     int nbAnt)
{
  // Sample history for channel impulse response - per-device (vrtsim_state), NOT a shared static,
  // so the relay's concurrent Uu + PC5 chanmod TX threads don't race on it.
  c16_t (*saved_samples)[SAVED_SAMPLES_LEN] = vrtsim_state->saved_samples;
  if (vrtsim_state->use_cirdb) {
  ...
  // Noise level is read LIVE from chan_desc->noise_power_dB inside the per-model loop below, so a runtime
  // change (telnet "channelmod modify <id> noise_power_dB <v>") takes effect on the next write.
  int num_chan_desc = 1;
  if (vrtsim_state->role == ROLE_SERVER) {
  ...
  for (int i = 0; i < num_chan_desc; i++) {
    ...
    // Live noise level from the channel descriptor (telnet "channelmod modify ... noise_power_dB" edits this).
    // INVALID_DBFS_VALUE (or above) means "noise off". Regenerate the noise device only when the level changes.
    double npdb = chan_desc->noise_power_dB;
    int16_t noise_power = (npdb >= (double)INVALID_DBFS_VALUE) ? 0 : (int16_t)(32767.0 / powf(10.0, .05 * -npdb));
    if (noise_power != vrtsim_state->cur_noise_power) {
      set_noise_device_power((float)noise_power);
      vrtsim_state->cur_noise_power = noise_power;
      LOG_I(HW, "VRTSIM: noise level updated -> noise_power_dB=%.1f (sample value %d)\n", npdb, noise_power);
    }
...
{
  randominit(0);
  vrtsim_state_t *vrtsim_state = calloc_or_fail(sizeof(vrtsim_state_t));
  vrtsim_state->cur_noise_power = -1; // force the first chanmod write to sync the noise device to channel_desc
  ...
  // Sidelink start hook (branch-only field, not in upstream vrtsim which is 1gNB-1UE only).
  // PC5 must use a SHARED-MEMORY CHANNEL DISTINCT from Uu, so it has its own connect that keys on
  // the *_sl config params (shm_channel_name_sl / connection_descriptor_sl / role_sl). nr-ue.c starts
  // the Uu device via trx_start_func and the separate rfdevice_sl via trx_start_func_sl.
  device->trx_start_func_sl = vrtsim_connect_sl;
  ...
  if (vrtsim_state->chanmod || vrtsim_state->taps_socket || vrtsim_state->use_cirdb) {
    init_channelmod();
    randominit(0);
    int noise_power_dBFS = vrtsim_state->noise_power_dBFS;

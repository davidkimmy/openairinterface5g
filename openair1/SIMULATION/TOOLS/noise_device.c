...
// Marsaglia SHR3 generator. Upstream provides this macro via sim.h; defined
// locally here because this branch's sim.h does not carry it.
#ifndef SHR3
...
void set_noise_device_power(float noise_power)
{
  // Runtime noise-level change: store the new level and regenerate all precomputed blocks.
  // Called only when the requested level changes (e.g. a telnet "channelmod modify ... noise_power_dB"),
  // not per sample, so the full-buffer regen cost is amortized.
  state.noise_power = noise_power;
  for (int i = 0; i < NUM_BLOCKS; i++)
    generate_one_block_noise(i, noise_power);
}

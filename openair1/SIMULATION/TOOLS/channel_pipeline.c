...
void channel_job(void *args)
{
  ...
  do_convolution_and_noise(common->nb_tx,
                           ...
                           common->num_jobs,
                           common->noise_power);
  // Completion is signalled by the thread pool returning this element to the
  // response FIFO (see channel_pipeline()); no explicit completion call needed.
}
                      ...
                      float noise_power)
{
  ...
  size_t num_tpool_threads = thread_pool->nbThreads;
  ...
  // Dispatch jobs via this branch's notifiedFIFO thread-pool API: each job is a
  // FIFO element whose msgData carries its job_args_t. After the worker runs
  // channel_job(), the pool returns the element to respFifo, which we drain to
  // wait for completion (replaces the upstream task_ans synchronization).
  const int total_jobs = nb_rx * num_jobs_per_rx_antenna;
  notifiedFIFO_t respFifo;
  initNotifiedFIFO(&respFifo);
  for (int aarx = 0; aarx < nb_rx; aarx++) {
    for (int job_idx = 0; job_idx < num_jobs_per_rx_antenna; job_idx++) {
      notifiedFIFO_elt_t *msg = newNotifiedFIFO_elt(sizeof(job_args_t), 0, &respFifo, channel_job);
      job_args_t *job_args = (job_args_t *)NotifiedFifoData(msg);
      ...
      pushTpool(thread_pool, msg);
    }
  }
  for (int i = 0; i < total_jobs; i++) {
    notifiedFIFO_elt_t *res = pullNotifiedFIFO(&respFifo);

...
// 12x12 IDFT tables for PUCCH format-0 correlation. Defined file-local here (NOT in a shared header):
// the gNB pucch_rx.c also defines these as file-local statics, so a header copy would clash.
                          ...
                          nfapi_nr_pucch_pdu_t *pucch_pdu)
{
  ...
  LOG_D(PHY,
        "[PSFCH RX] %d.%d ics=%d maxpos=%d xrtmag=%ld xrtmag_next=%ld nseq=%d -> HARQ %s\n",
        frame, slot, pucch_pdu->initial_cyclic_shift, maxpos, (long)xrtmag, (long)xrtmag_next, nr_sequences,
        ack_nack == 0 ? "ACK" : "NACK");
  return ack_nack;
}

...
#include "common/utils/colors.h"  // KRED for the SL sync-progress lines
...
static int sl_nr_pss_correlation(PHY_VARS_NR_UE *UE, int frame_index)
{
  ...
  LOG_I(NR_PHY,
        "%sPSS Source = %d, Peak found at pos %d, val = %llu (%d dB) avg %d dB, ffo %lf, freq offset:%d Hz\n",
        KRED,
        pss_source,
        peak_position,
                              ...
                              c16_t rxdataF[][ue->SL_UE_PHY_PARAMS.sl_frame_params.samples_per_slot_wCP])
{
  ...
  *tot_metric = INT_MIN;
  ...
  LOG_I(NR_PHY,
        "%sUE[%d]NR-SL SLSS SEARCH: SSS Processing over. id2 from SSS:%d, id1 from PSS:%d, SLSS id:%d\n",
        KRED,
        ue->Mod_id,
...
nr_initial_sync_t sl_nr_slss_search(PHY_VARS_NR_UE *UE, UE_nr_rxtx_proc_t *proc, int num_frames)
{
  ...
  for (int frame_index = 0; frame_index < num_frames; frame_index++) {
    ...
    for (int pss_sym = 1; pss_sym < 3; pss_sym++) {
      ...
      LOG_I(NR_PHY,
            "%sUE[%d]SIDELINK SEARCH SLSS: PSS Peak at %d, PSS sym:%d, Estimated PSS position %d\n",
            KRED,
            UE->Mod_id,
            sync_pos,
            pss_sym,
      ...
      if (ssb_end_position < num_frames * sl_fp->samples_per_frame) {
        ...
        if (UE->UE_fo_compensation) { // This code to be checked. Why do we do this before PSS detection is successful?
          ...
          LOG_I(NR_PHY,
                "%sSLSS SEARCH: FREQ comp of SLSS samples. Freq_OFSET:%d, startpos:%d, end_pos:%d\n",
                KRED,
                sync_params->freq_offset,
                start,
                end);
        ...
        if (result.cell_detected) { // Check this later TBD
          ...
          LOG_I(NR_PHY,
                "%sUE[%d]SIDELINK SLSS SEARCH: PSBCH RX OK. Remainingframes:%d, rx_offset:%d\n",
                KRED,
                UE->Mod_id,
                sync_params->remaining_frames,
          ...
          LOG_I(NR_PHY,
                "%sUE[%d]SIDELINK SLSS SEARCH: SL-MIB: DFN:%d, slot:%d.\n",
                KRED,
                UE->Mod_id,
                sync_params->DFN,

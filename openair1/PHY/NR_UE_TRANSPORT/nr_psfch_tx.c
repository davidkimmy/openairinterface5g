                        ...
                        const sl_nr_tx_rx_config_psfch_pdu_t *psfch_pdu)
{
  ...
  // PSFCH is transmitted on the PC5 (sidelink) interface: pass PC5 so nr_generate_pucch0 uses the
  // sidelink group/sequence hopping and single-symbol cyclic shift (38.211 sec 8.x), not the Uu path.
  (void)ue;
  nr_generate_pucch0(txdataF, frame_parms, amp, nr_slot_tx, &pucch_pdu, PC5);
}

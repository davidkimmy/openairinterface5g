...
static void nr_sdap_rx_entity(nr_sdap_entity_t *entity,
  ...
  if (is_gnb) { // gNB
    ...
    if (nas_sock_fd[index] <= 0) {
      LOG_E(SDAP, "invalid nas_sock_fd[%d]=%d: dropping %d bytes for ue_id=0x%04lx\n",
            index, nas_sock_fd[index], size-offset, ue_id);
      return;
    }
    ...
    if (len != size-offset) {
      LOG_E(SDAP, "FATAL: write failed! Expected %d bytes, wrote %d bytes to nas_sock_fd[%d]=%d, errno=%d (%s)\n",
            size-offset, len, index, nas_sock_fd[index], errno, strerror(errno));
    } else {
      LOG_D(SDAP, "[TUN_WRITE_SUCCESS] Wrote %d bytes to nas_sock_fd[%d] (oaitun_ue%d) for ue_id=0x%04lx\n",
            len, index, index+1, ue_id);
    }
  }
}
...
nr_sdap_entity_t *new_nr_sdap_entity(int is_gnb, bool has_sdap_rx, bool has_sdap_tx, ue_id_t ue_id, int pdusession_id, bool is_defaultDRB, uint8_t drb_identity, NR_QFI_t *mapped_qfi_2_add, uint8_t mappedQFIs2AddCount)
{
  if (nr_sdap_get_entity(ue_id, pdusession_id)) {
    LOG_W(SDAP, "SDAP Entity for UE already exists with RNTI/UE ID: %lu and PDU SESSION ID: %d - updating configuration\n", ue_id, pdusession_id);
    nr_sdap_entity_t *existing_sdap_entity = nr_sdap_get_entity(ue_id, pdusession_id);

    /* Do NOT update default DRB if one is already set - default should remain stable
       Additional DRBs should NOT change the default (they should have is_defaultDRB=false) */
    if (is_defaultDRB && existing_sdap_entity->default_drb == 0) {
      existing_sdap_entity->default_drb = drb_identity;
      LOG_D(SDAP, "Set default DRB to %d for existing SDAP entity (UE %lu, PDU Session %d)\n",
            drb_identity, ue_id, pdusession_id);
    } else if (is_defaultDRB) {
      LOG_W(SDAP, "Ignoring defaultDRB flag for DRB %d - default DRB %ld already set for UE %lu, PDU Session %d\n",
            drb_identity, existing_sdap_entity->default_drb, ue_id, pdusession_id);
    }

    // Get PDCP entity for QFI-to-DRB mapping update
    rb_id_t pdcp_entity = existing_sdap_entity->default_drb;

    /* Update QFI-to-DRB mapping for both gNB and UE
       This is needed when PDU session is re-established with new DRB configuration */
    nr_sdap_ue_qfi2drb_config(existing_sdap_entity, pdcp_entity, ue_id, mapped_qfi_2_add,
                               mappedQFIs2AddCount, drb_identity, has_sdap_rx, has_sdap_tx);

    LOG_D(SDAP, "SDAP entity configuration updated for UE %lu, PDU Session %d (gNB=%d, DRB=%d)\n",
          ue_id, pdusession_id, is_gnb, drb_identity);
